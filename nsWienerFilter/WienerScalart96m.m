...
W=fix(.025*fs);                             % 帧长为25ms,W为这25ms内所取点数求整
SP = .4; %帧移比例取40%（10ms），帧移+overLap = 帧长
wnd = hamming(W); %设置窗函数
...
NIS = fix((IS*fs-W)/(SP*W)+1); %计算无话段(IS)帧数
y = segment(signal,W,SP,wnd); %分帧
Y = fft(y); %FFT
YPhase = angle(Y(1:fix(end/2)+1,:)); %带噪语音的相位角
Y = abs(Y(1:fix(end/2)+1,:)); %取正频率谱值
numberOfFrame = size(Y,2); %计算总帧数
FreqResol = size(Y,1); %计算频谱中的横轴频率点数
...
for i=1:numberOfFrame  
    ...
    if SpeechFlag == 0  %在无话段中平滑更新噪声谱值
        ...
        LambdaD = (NoiseLength*LambdaD+(Y(:,i).^2))./(1+NoiseLength); %更新平滑噪声方差
    end
...
xi=alpha*(G.^2).*Gamma+(1-alpha).*max(gammaNew-1,0); % 计算先验信噪比
    ...
    X(:,i) = G.*Y(:,i); %维纳滤波后的幅值
    waitbar(i/numberOfFrame,h,num2str(fix(100*i/numberOfFrame)));%显示运行进度条图
end
close(h); %关闭进度条图
